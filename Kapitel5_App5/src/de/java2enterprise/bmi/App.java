...
public class App {
	...
	public App() {
		...
		if(optional.isPresent()) {
			katzen
				.stream()
				.filter(k->(!k.equals(optional.get())))
				.forEach(System.out::println);
			
		}
		
		//Gib den durchschnittlichen BMI-Wert der Katzen aus
		katzen
			.stream()
			.mapToDouble(k->{
				double groesse = k.getMessung().getGroesse();
				double gewicht = k.getMessung().getGewicht();
