...
public class App {
	...
	public App() {
		...
		katzen.forEach(System.out::println);
		
		//Gebe die Katzen sortiert nach der Größe aus
		Collections.sort(katzen, comparator);
		katzen.forEach(System.out::println);
		
		//Gebe die Größe der größten Katze aus
		Katze groessteKatze = Collections.max(katzen, comparator);
		System.out.println(groessteKatze.getMessung().getGroesse());
		
		//Entferne die größte Katze und gebe die Katzen erneut aus,
		//falls eine Katze entfernt wurde.
		if(katzen.removeIf(k->(k.equals(groessteKatze)))) {
			katzen.forEach(System.out::println);
		}
	}
}
